...
//     <div className={`${!store.expandModel && 'pointer-events-none'} select-none flex lg:w-2/5 h-[100dvh] flex-col justify-center items-center`}> {/*  THIS NEEDS ATTENTION */}
...
import PriceContainer from "./MenuComponents/PriceContainer";
 ...
 export default function ModelViewer({ canvasRef }: { canvasRef: React.RefObject<HTMLDivElement | null> }) {
  ...
  return window.innerWidth >= 1024 && (
    <div className={`${!store.expandModel && 'pointer-events-none'} select-none flex lg:w-2/5 h-[100dvh] flex-col justify-center items-center`}>
      <div className="w-3/4 3xl:w-3/5 py-6 xl:py-10 3xl:pt-10 3xl:pb-4 shadow-md rounded-xl bg-[#F5F8FB] h-[90%] flex p-4 flex-col gap-4">
        <MenuHeader />
        <div className="w-full grow overflow-y-auto rounded-md flex flex-col gap-2">
          <MenuContainer />
        ...
        <hr className='mt-5 mb-1 border border-[#0000002E] w-[95%] mx-auto' />
        <PriceContainer configRef={canvasRef} /> {/* ✅ pass it here */}
      </div>
    </div>
  );
}
